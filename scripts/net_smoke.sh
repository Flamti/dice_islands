...
# Сценарий 2 (этапы 2–7): партия двух людей — барьер PhaseReady, таймеры фаз,
# у каждого участника свой остров с сеткой, кубик замка и реролл (кресты
# заблокированы), гость строит ферму (повтор отклоняется), ферма видна обоим,
# система опасности активна и лента событий партии доходит по сети.
set -euo pipefail
...
echo "OK: сетевые смоук-тесты пройдены (лобби; партия: фазы, острова, стройка, кубики, опасность)"
